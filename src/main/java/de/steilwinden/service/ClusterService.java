...
import de.steilwinden.service.data.Cluster;
import de.steilwinden.service.data.Point;
...
public class ClusterService {
    ...
    /**
     * 4 neighbours need to be investigated:
     * 3 2 1
     * 4 X
     */
    ...
    private void print(List<Cluster> clusterList, int imageWidth, int imageHeight) {
        ...
        for (int y = 0; y < imageHeight; y++) {
            ...
            log.info("cluster(%02d)=%s", y, print);
        }
    }
    ...
    private BufferedImage cropImageByCluster(BufferedImage image, Cluster cluster, int imageWidth, int imageHeight) {
        ...
        if (startX < 0 || startX + 2 * halfLength > imageWidth || startY < 0 || startY + 2 * halfLength > imageHeight) {
            throw new IllegalArgumentException("Digit is too close to the edge of the picture. "
                    + "startX=" + startX + " startY=" + startY);
        }

        log.info("startX: " + startX);
        log.info("startY: " + startY);
        log.info("halfLength: " + halfLength);
    ...
    private void print(BufferedImage image) {
        ...
        for (int y = 0; y < image.getHeight(); y++) {
            ...
            log.info("subimage(%02d)=%s", y, print);
        }
    }
}
